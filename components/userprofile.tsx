...
const UserProfile = () => {
    ...
    const {isPending, data} = useQuery({
        queryKey: ["profile"],
        queryFn: async () =>{
            const res = await axios.get('/api/user?id='+session.data?.user.id);
                ...
                <div>
                    <div className="relative group mb-10">
                        <Avatar className="border mx-auto w-[100px] h-[100px]">
                            <AvatarImage src={data?.image} alt={data?.username} width={100} height={100}/>
                    ...
                    <form>
                        ...
                        <div className="my-2">
                            ...
                            {errors.username && <FadeText className="text-red-500 font-semibold text-xs" text={errors.username.message}/>}
                        </div>
                        <div className="my-2">
                            <div className="my-2">
                                <Input type="password" placeholder="password" {...register("password")}/>
                                {errors.password && <FadeText className="text-red-500 font-semibold text-xs" text={errors.password.message}/>}
                            </div>
                            <div className="my-2">
                                <Input type="password" placeholder="Confirm Password" {...register("confirmPassword")}/>
                                {errors.confirmPassword && <FadeText className="text-red-500 font-semibold text-xs" text={errors.confirmPassword.message}/>}
                            </div>
                        </div>
                        <div className="my-2">
                            <Button type="submit" variant="outline" className="w-full">Update Profile</Button>
                        </div>
                    </form>
                </div>
            </SheetContent>
